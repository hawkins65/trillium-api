                ...
                try:
                    ...
                    if response_block.status_code == 200:
                        ...
                        if "result" in response_json:
                            ...
                            if debug: logger.debug(f"Epoch {epoch_number} Thread {thread_id} - Slot {slot}: Block info retrieved successfully")
                            slot_data_entry = extract_slot_data(slot, block_info, epoch_number)
                            if slot_data_entry:
                                slot_writer.writerow(slot_data_entry)
                                if debug: logger.debug(f"Epoch {epoch_number} Thread {thread_id} - Slot {slot}: Slot data written to CSV")
                            else:
                            ...
                            if debug: logger.debug(f"Epoch {epoch_number} Thread {thread_id} - Slot {slot}: Vote data written to CSV")
                            break
                        else:
                            error_code = response_json.get("error", {}).get("code")
                            log_error(slot, error_code, response_json)
                            if debug: logger.debug(f"Epoch {epoch_number} Thread {thread_id} - Slot {slot}: Error code {error_code}")
                        ...
                        if debug: logger.debug(f"Epoch {epoch_number} Thread {thread_id} - Slot {slot}: Response status code {response_block.status_code}")
                        retry_count += 1
                        time.sleep(retry_delay)
                        retry_delay *= 2
