# trillium-discord-update-channel-with-stake.py
...
# 2) Fallback: ~/api/.env.<basename>
if not env_file.exists():
    env_file = Path.home() / 'api' / f'.env.{script_basename}'
...
if not env_file.exists():
    sys.exit(
        ...
        f"  {Path.home()}/api/.env.{script_basename}"
    )
...
if not bot_token:
    ...
    logging.info(f"Successfully read BOT_TOKEN ending in {bot_token[-5:]}")
...
async def on_ready():
    ...
    logging.debug(f"Bot guild-wide permissions: {role_permissions}")
    logging.debug(f"Guild Manage Channels: {role_permissions.manage_channels}")
    ...
    logging.debug(f"Bot permissions for channel {channel.id}: {permissions}")
    if not permissions.manage_channels:
        logging.error("Bot lacks 'Manage Channels' permission for this channel! ❌")
    ...
    if channel.category:
        ...
        logging.debug(f"Bot permissions for category {channel.category.name} (ID: {channel.category.id}): {category_permissions}")
        logging.debug(f"Category Manage Channels: {category_permissions.manage_channels}")
