    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            if not epoch_result or epoch_result[0][0] is None:
                logger.error("Could not determine latest epoch")
                return
            ...
            while True:
                try:
                    ...
                    if epoch <= 616:
                        logger.info("Please enter a valid epoch number greater than 615")
                        continue
                    break
                except ValueError:
                    logger.info("Please enter a valid epoch number")
...
Processing historical data from epoch {start_epoch} to {epoch}...")
            ...
            for hist_epoch in range(start_epoch, epoch + 1):
                logger.info(f"Processing historical epoch: {hist_epoch}")
        ...
        logger.info(f"Total blame score: {total_blame_score}")
        logger.info(f"Validators with blame: {validators_with_blame}")
        logger.info(f"Average blame score: {current_summary['average_blame_score']}")
        logger.info(f"Median blame score: {current_summary['median_blame_score']}")

        if PROCESS_HISTORICAL_SUMMARY:
            logger.info(f"\nHistorical Summary (Epochs {start_epoch} to {epoch}):")
            logger.info(f"Average total blame score: {historical_summary['aggregate_statistics']['average_total_blame_score']}")
            logger.info(f"Average validators with blame: {historical_summary['aggregate_statistics']['average_validators_with_blame']}")
            logger.info(f"Average blame score: {historical_summary['aggregate_statistics']['average_blame_score']}")
            logger.info(f"Average median score: {historical_summary['aggregate_statistics']['average_median_score']}")
        ...
        if total > 0:
            ...
            logger.info(f"Divisible by 4: {div_by_4} ({(div_by_4/total*100):.1f}%)")
            logger.info(f"Divisible by 3: {div_by_3} ({(div_by_3/total*100):.1f}%)")
            logger.info(f"Divisible by 2 only: {div_by_2_only} ({(div_by_2_only/total*100):.1f}%)")
            logger.info(f"Not divisible by 2,3,4: {other} ({(other/total*100):.1f}%)")
