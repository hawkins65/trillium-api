...
logger.info(f"Epoch number: {epoch_number}")
...
for index, run_directory in enumerate(run_directories, start=1):
    ...
    if count_difference > 10:
        logger.warning(f"Warning: The difference between the number of epoch_votes and slot_data CSV files is: {count_difference}.")
        user_input = input("Do you want to continue? (y/n) [y]: ").strip().lower() or 'y'
        if user_input != 'y':
            logger.info("Exiting the script.")
            exit()

    logger.info("Process epoch_votes CSV files")
    ...
    logger.info("Process slot_data CSV files")
    for slot_data_file in slot_data_files:
        with open(slot_data_file, "r") as file:
            ...
            logger.info(f"Processing {slot_data_file} - {csv_row_count} rows")
    ...
    if total_size == 8192:
        logger.info("Skipping to the next run directory due to empty temp tables.")
        continue
...
logger.info(f"Total rows in CSV files: {total_csv_rows}")
logger.info(f"Rows in temp_validator_data: {temp_table_count}")

if total_csv_rows != temp_table_count:
    logger.warning(f"Warning: Mismatch in row counts. Difference: {total_csv_rows - temp_table_count}")
else:
    logger.info("Row counts match between CSV files and temp_validator_data table.")

# Additional checks
cursor.execute("SELECT COUNT(DISTINCT block_slot) FROM temp_validator_data")
distinct_slots = cursor.fetchone()[0]
logger.info(f"Distinct block_slots in temp_validator_data: {distinct_slots}")
...
if potential_duplicates:
    logger.warning("Top 5 potential duplicate block_slots in temp_validator_data:")
    for slot, count in potential_duplicates:
        logger.warning(f"  Block slot {slot}: {count} occurrences")
...
logger.info("Walk the slot chain list to find any missing parent slots and duplicate slots")
...
with open(log_file, 'r') as log:
    ...
    if log_contents.strip():
        ...
        if user_input == 'y':
            logger.info("Proceeding with the data loading process...")
            error_found = False
        else:
            logger.info("Aborting the data loading process.")
    else:
        logger.info(f"No errors found in {log_file}. Proceeding with the data loading process...")
...
if not error_found:
    ...
    logger.info(f"Rows in validator_data for epoch {epoch_number} before insert: {before_insert}")

    logger.info("Delete existing data for the epoch")
    cursor.execute(f"DELETE FROM validator_data WHERE epoch = {epoch_number}")

    logger.info("Copy data from the temporary table to the validator_data table")
    cursor.execute("""
    ...
    logger.info(f"Rows in validator_data for epoch {epoch_number} after insert: {after_insert}")

    logger.info(f"Difference: {after_insert - before_insert}")
    ...
    logger.info(f"Data loaded successfully into validator_data")
else:
    logger.error(f"Errors found during temp_validator_data parent_slot chain check. Data loading aborted. Check the log file '{log_file}' for details.")
...
if not error_found:
    ...
    logger.info("Insert aggregated vote counts into the temporary table")
    cursor.execute("""
    ...
    logger.info("Merge data from the temporary table into the validator_stats table, choosing the rows where votes_cast is large and ignoring any rows where votes_cast <=5")
    logger.info(f"Starting merge for epoch {epoch_number}: Merging data from temp_validator_stats into validator_stats.")
    logger.info("Choosing rows with votes_cast > 5 and prioritizing highest votes_cast.")
    ...
    try:
        ...
        logger.info(f"Epoch {epoch_number}: Step 1 - Staging authoritative data completed.")
        ...
        logger.info(f"Epoch {epoch_number}: Step 2 - Deletion of outdated conflicting vote_account associations completed. ({cursor.rowcount} rows deleted)")
        ...
        logger.info(f"Epoch {epoch_number}: Step 3 - Upsert into validator_stats completed. ({cursor.rowcount} rows affected/inserted)")

        # Commit the transaction
        conn.commit()
        logger.info(f"Epoch {epoch_number}: Data loaded successfully into validator_stats and transaction committed.")

    except psycopg2.Error as e:
        logger.error(f"Epoch {epoch_number}: A database error occurred: {e}")
        logger.error(f"SQL Error Code: {e.pgcode}")
        logger.error(f"Details: {e.pgerror}")
        if conn:
            conn.rollback()
        logger.error(f"Epoch {epoch_number}: Transaction rolled back.")
    except Exception as e:
        logger.error(f"Epoch {epoch_number}: A non-database error occurred: {e}")
        if conn:
            conn.rollback()
        logger.error(f"Epoch {epoch_number}: Transaction rolled back due to non-database error.")
    finally:
        ...
        if cursor:
            try:
                ...
                logger.info(f"Epoch {epoch_number}: Step 4 - Temporary table temp_final_validator_data dropped.")
            except psycopg2.Error as drop_e:
                logger.error(f"Epoch {epoch_number}: Error dropping temporary table: {drop_e}")
            # No need to rollback here if drop fails, main transaction is already handled.
else:
    logger.info(f"Errors found during temp_validator_data parent_slot chain check. Epoch vote data loading skipped for epoch {epoch_number}.")
...
for json_file in json_files:
    ...
    if not os.path.exists(file_path):
        logger.warning(f"Warning: {json_file} not found in {run0_directory}")
        continue
    ...
    epoch = data['data_epochs'][1]  # Get the second element of data_epochs
...
try:
    ...
    """, all_data)
    conn.commit()
    logger.info(f"Successfully processed run0 JSON files and upserted {len(all_data)} rows into validator_xshin table")
except psycopg2.Error as e:
...
logger.info(f"Time for section 1 setup:                           {elapsed_time_1:.2f} seconds ({format_time(elapsed_time_1)} minutes:seconds)")
logger.info(f"Time for section 2 slot_data chain verification:    {elapsed_time_2:.2f} seconds ({format_time(elapsed_time_2)} minutes:seconds)")
logger.info(f"Time for section 3 slot data finalize:              {elapsed_time_3:.2f} seconds ({format_time(elapsed_time_3)} minutes:seconds)")
logger.info(f"Time for section 4 processing epoch votes:          {elapsed_time_4:.2f} seconds ({format_time(elapsed_time_4)} minutes:seconds)")
logger.info(f"Time for section 5 slot data finalize:              {elapsed_time_5:.2f} seconds ({format_time(elapsed_time_5)} minutes:seconds)")
logger.info(f"Total processing time:                              {total_elapsed_time:.2f} seconds ({format_time(total_elapsed_time)} minutes:seconds)")