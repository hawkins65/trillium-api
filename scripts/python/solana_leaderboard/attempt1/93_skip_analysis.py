...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            ...
            logger.error(f"Error: Invalid epoch number '{sys.argv[1]}'")
            sys.exit(1)
    else:
        ...
        while True:
            try:
                ...
                if epoch <= 0:
                    logger.info("Please enter a valid epoch number greater than 0")
                    continue
                break
            except ValueError:
...
Processing epoch: {epoch}")
    ...
    if not identity_pubkeys:
        logger.info(f"No validators found for epoch {epoch}")
        return
...
Epoch {epoch} Overview:")
    ...
    logger.info(f"Total produced: {total_slots - total_skipped:,}")
    logger.info(f"Total missed: {total_skipped:,}")
    logger.info(f"Total validators: {len(identity_pubkeys):,}")
    logger.info(f"Overall skip percentage: {skip_percentage:.2f}%")
