...
def store_validators():
    ...
    try:
        ...
        if not validators:
            print("No validator data to store.")
            return
        ...
        for validator in validators:
            ...
            if "epoch" not in validator or validator["epoch"] is None:
                print(f"Skipping validator {validator.get('identity', 'unknown')} due to missing epoch")
                continue
            ...
            if len(data) != 69:
                print(f"Error: Data tuple has {len(data)} values, expected 69 for validator {validator.get('identity', 'unknown')}")
            ...
            try:
                ...
                print(f"Database error for validator {validator.get('identity', 'unknown')}: {e}")
                print(f"Validator data: {validator}")
                conn.rollback()
                continue
        ...
        conn.commit()
        print(f"Successfully stored {inserted_count} validator records")
