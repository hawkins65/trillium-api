...
try:
    log_with_epoch(f"Reading validator JSON file: {validator_filename}", epoch)
    with open(validator_filename, 'r') as f:
        validator_data = json.load(f)
except Exception as e:
    log_with_epoch(f"Error reading validator JSON file: {e}", epoch, "error")
    sys.exit(1)

# Read award winners JSON file
award_filename = f"90_xshin_all_award_winners_{epoch}.json"
try:
    log_with_epoch(f"Reading award winners JSON file: {award_filename}", epoch)
    with open(award_filename, 'r') as f:
        award_data = json.load(f)
except Exception as e:
    log_with_epoch(f"Error reading award winners JSON file: {e}", epoch, "error")
    sys.exit(1)
...
for award in award_data:
    ...
    for winner in award['winners']:
        ...
        try:
            ...
            log_with_epoch(f"Error processing award for pubkey {pubkey}, category {category}: {e}", epoch, "error")
            continue
...
try:
    ...
    log_with_epoch(f"Error connecting to database: {e}", epoch, "error")
    sys.exit(1)

# Insert or update data
try:
    log_with_epoch(f"Inserting/updating {len(validator_data)} validator records into xshin_data for epoch {epoch}", epoch)
    for validator in validator_data:
        ...
        try:
            ...
            log_with_epoch(f"Error processing stake data for pubkey {vote_account_pubkey}: {e}", epoch, "error")
            continue
        ...
        for category in awards:
            if category in award_columns:
                ...
                log_with_epoch(f"Unknown award category {category} for pubkey {vote_account_pubkey}", epoch, "error")
        ...
        if len(values) != 74:
            log_with_epoch(f"Invalid tuple length for pubkey {vote_account_pubkey}: expected 74, got {len(values)}", epoch, "error")
            continue
    ...
    conn.commit()
    log_with_epoch(f"Successfully processed validator data for epoch {epoch}", epoch)

except Exception as e:
    log_with_epoch(f"Error processing validator data: {e}", epoch, "error")
    conn.rollback()
finally:
    if 'cur' in locals():
        cur.close()
