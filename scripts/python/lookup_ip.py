...
def read_ip_file(file_path):
    if not os.path.exists(file_path):
        logger.error(f"Input file '{file_path}' not found. Please provide a file with one IP address per line.")
        sys.exit(1)
    try:
        with open(file_path, 'r') as f:
            # Strip whitespace and filter out empty lines
            ips = [line.strip() for line in f if line.strip()]
        logger.info(f"Read {len(ips)} IP addresses from {file_path}")
...
def update_geoip_data(conn, results):
    if not results:
        logger.info("No GeoIP data to update")
        return
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.info(f"Updated {len(results)} IP records in validator_stats")
    except Exception as e:
        logger.error(f"Error updating database: {e}")
        conn.rollback()

def process_ips(ips, session, conn):
    results = []
    if not ips:
        logger.info("No IPs to process")
        return results

    batches = [ips[i:i + MAX_BATCH_SIZE] for i in range(0, len(ips), MAX_BATCH_SIZE)]
...
def main():
    ...
    try:
        ...
        # Process IPs and get GeoIP data
        results = process_ips(ips, session, conn)
        
        # Update database with GeoIP data
        update_geoip_data(conn, results)
        
    finally:
        session.close()
        conn.close()
        logger.info("HTTP session and database connection closed")
