...
def fetch_all_unique_ips(conn):
    ...
    with conn.cursor() as cur:
        ...
        logger.info(f"Total unique IPs found: {len(ips)}")
        return ips

def update_geographic_data_for_ips(ips, conn, session):
    logger.info(f"Processing {len(ips)} unique IPs")
    
    if not ips:
        logger.info("No IPs found in validator_stats")
        return
    ...
    if results:
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.info(f"Updated {len(results)} IPs in validator_stats")
    else:
        logger.info("No geographic data to update")
...
def main():
    ...
    try:
        unique_ips = fetch_all_unique_ips(conn)
        update_geographic_data_for_ips(unique_ips, conn, session)
    finally:
        conn.close()
