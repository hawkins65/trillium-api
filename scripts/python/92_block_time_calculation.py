...
def calculate_block_times(sequences: List[List[tuple]]) -> Tuple[float, float]:
    ...
    logging.info(f"Found {len(sequence_averages)} valid sequence averages")
...
def analyze_blocks_by_epoch(cur, conn, target_epoch: int = None) -> Dict[int, Dict[str, float]]:
    logging.info("Starting block analysis...")
    ...
    logging.info(f"Processing {total_epochs} epochs in batches of {BATCH_SIZE}")
    
    for epoch_start in range(min_epoch, max_epoch + 1, BATCH_SIZE):
        epoch_end = min(epoch_start + BATCH_SIZE - 1, max_epoch)
        logging.info(f"Processing epochs {epoch_start} to {epoch_end}")
        ...
        logging.info(f"Retrieved {len(data)} blocks for processing")
        ...
        for epoch, epoch_blocks in epoch_data.items():
            ...
            logging.info(f"Epoch {epoch}: Found {len(sequences)} consecutive block sequences")
