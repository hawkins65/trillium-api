...
class BincodeParser:
    ...
    def read_string(self, max_bytes=256):
        ...
        if length > max_bytes:
            logger.warning(f"String length {length} exceeds max {max_bytes} at offset {self.offset}")
            length = max_bytes
        if length > len(self.buffer) - self.offset:
            logger.warning(f"String length {length} too high at offset {self.offset}")
            return None
        ...
        try:
            ...
            if len(encoded) > max_bytes:
                ...
                logger.warning(f"Truncated string to {max_bytes} bytes at offset {self.offset - length}: {decoded}")
            elif len(encoded) != length:
                logger.warning(f"String byte length mismatch at offset {self.offset - length}: {decoded}")
            return decoded
        except UnicodeDecodeError as e:
            logger.error(f"Invalid UTF-8 string at offset {self.offset - length}: {e}")
            return None
    ...
    def skip_string(self):
        ...
        if length > len(self.buffer) - self.offset:
            logger.warning(f"String length {length} too high at offset {self.offset}")
            return None
        logger.info(f"Skipping string of length {length} at offset {self.offset} for validator {self.current_validator}, field {self.current_field}")
        self.advance(length)
        return None
...
def bincode_pool_details(parser):
    ...
    try:
        ...
        logger.info(f"Pool version: {version}")
        if version not in (0, 1, 2):
            raise ValueError(f"unexpected version: {version}")
    except Exception as e:
        logger.error(f"Error parsing version at offset {parser.offset}: {e}")
        raise
    ...
    try:
        ...
        logger.info(f"Pool validator count: {pool_validator_count}")
    except Exception as e:
        logger.error(f"Error parsing pool_validator_count at offset {parser.offset}: {e}")
        raise
    ...
    for i in range(9):
        ...
        try:
            ...
            logger.info(f"Best metric {i} count: {count}")
            if count * (32 + 8 + 8) > len(parser.buffer) - parser.offset:
                logger.warning(f"Best metric {i} count {count} too high at offset {parser.offset}")
            ...
            logger.error(f"Error parsing best_metric_{i} at offset {parser.offset}: {e}")
            raise
    ...
    if version >= 1:
        ...
        try:
            ...
            logger.info(f"Best vote inclusion count: {count}")
            if count * (32 + 8 + 8) > len(parser.buffer) - parser.offset:
                logger.warning(f"Best vote inclusion count {count} too high at offset {parser.offset}")
                count = min(count, (len(parser.buffer) - parser.offset) // (32 + 8 + 8))
            for _ in range(count):
                parser.read_pubkey()
                parser.read_f64()
            ...
            logger.error(f"Error parsing best_vote_inclusion at offset {parser.offset}: {e}")
            raise
    ...
    try:
        ...
        logger.info(f"Best APY count: {count}")
        if count * (32 + 4 + 8) > len(parser.buffer) - parser.offset:
            logger.warning(f"Best APY count {count} too high at offset {parser.offset}")
        ...
        logger.error(f"Error parsing best_apy at offset {parser.offset}: {e}")
        raise
    ...
    for i in range(2):
        ...
        try:
            ...
            logger.info(f"Best concentration {i} count: {count}")
            if count * (32 + 8 + 8) > len(parser.buffer) - parser.offset:
                logger.warning(f"Best concentration {i} count {count} too high at offset {parser.offset}")
                count = min(count, (len(parser.buffer) - parser.offset) // (32 + 8 + 8))
            for _ in range(count):
                parser.read_pubkey()
                parser.read_f64()
            ...
            logger.error(f"Error parsing best_concentration_{i} at offset {parser.offset}: {e}")
            raise
    ...
    try:
        ...
        logger.info(f"Pool voter count: {count}")
        remaining_bytes = len(parser.buffer) - parser.offset
        if count * 32 > remaining_bytes:
            logger.warning(f"Pool voter count {count} too high for {remaining_bytes} bytes")
            count = min(count, remaining_bytes // 32)
    except Exception as e:
        logger.error(f"Error parsing voter_count at offset {parser.offset}: {e}")
        raise
    ...
    for i in range(count):
        ...
        try:
            ...
            logger.error(f"Error reading pubkey for pool voter {i+1}/{count}: {e}")
        ...
        try:
            ...
            logger.info(f"Parsed pool validator {pubkey}, bytes consumed: {parser.offset - start_offset}")
        except Exception as e:
            logger.error(f"Skipping pool validator {pubkey} due to parsing error at offset {parser.offset}: {e}")
...
def bincode_non_pool_voters(parser):
    ...
    try:
        ...
        logger.info(f"Non-pool voters version: {version}")
        if version not in (0, 1, 2):
            raise ValueError(f"unexpected version: {version}")
    except Exception as e:
        logger.error(f"Error parsing version at offset {parser.offset}: {e}")
    ...
    try:
        ...
        logger.info(f"Non-pool voter count: {count}")
        remaining_bytes = len(parser.buffer) - parser.offset
        if count * 32 > remaining_bytes:
            logger.warning(f"Non-pool voter count {count} too high for {remaining_bytes} bytes")
            count = min(count, remaining_bytes // 32)
    except Exception as e:
        logger.error(f"Error parsing voter_count at offset {parser.offset}: {e}")
        raise
    ...
    for i in range(count):
        ...
        try:
            ...
            logger.error(f"Error reading pubkey for non-pool voter {i+1}/{count}: {e}")
        ...
        try:
            ...
            logger.info(f"Parsed non-pool validator {pubkey}, bytes consumed: {parser.offset - start_offset}")
        except Exception as e:
            logger.error(f"Skipping non-pool validator {pubkey} due to parsing error at offset {parser.offset}: {e}")
...
def bincode_pool_voter_details(parser, version):
    ...
    logger.info(f"Parsing voter_details at offset {parser.offset} for validator {parser.current_validator}")
    try:
        details = bincode_voter_details(parser, version, is_pool=True)
    except Exception as e:
        logger.error(f"Error parsing voter_details for validator {parser.current_validator} at offset {parser.offset}: {e}")
        return None
    parser.current_field = "pool_stake"
    try:
        pool_stake = bincode_stake(parser)
    except Exception as e:
        logger.error(f"Error parsing pool_stake for validator {parser.current_validator} at offset {parser.offset}: {e}")
    ...
    try:
        ...
        if count * 8 > len(parser.buffer) - parser.offset:
            logger.warning(f"Noneligibility count {count} too high for validator {parser.current_validator}")
            count = 0
    except Exception as e:
        logger.error(f"Error parsing noneligibility_count at offset {parser.offset}: {e}")
        count = 0
    ...
    for _ in range(count):
        try:
            ...
            logger.error(f"Error parsing noneligibility reason for validator {parser.current_validator} at offset {parser.offset}: {e}")
            break
    parser.current_field = None
    return {
        'details': details,
...
def bincode_voter_details(parser, version, is_pool):
    ...
    for field in ['name', 'icon_url', 'details', 'website_url', 'city', 'country']:
        ...
        logger.info(f"Parsing {field} at offset {parser.offset}")
        try:
            if parser.read_bool():
            ...
            logger.error(f"Parse failed for {field}, setting to None at offset {parser.offset}: {e}")
            print(f"Parse failed for {field}, setting to None: {e}")
            result[field] = None
    
    parser.current_field = "stake"
    logger.info(f"Parsing stake at offset {parser.offset}")
    try:
        result['stake'] = bincode_stake(parser)
    except Exception as e:
        logger.error(f"Error parsing stake at offset {parser.offset}: {e}")
        result['stake'] = {'active': 0, 'activating': 0, 'deactivating': 0}

    parser.current_field = "target_pool_stake"
    logger.info(f"Parsing target_pool_stake at offset {parser.offset}")
    try:
        result['target_pool_stake'] = parser.read_u64()
    except Exception as e:
        logger.error(f"Error parsing target_pool_stake at offset {parser.offset}: {e}")
        result['target_pool_stake'] = 0

    parser.current_field = "raw_score"
    logger.info(f"Parsing raw_score at offset {parser.offset}")
    try:
        result['raw_score'] = bincode_score(parser, version, is_pool)
    except Exception as e:
        logger.error(f"Error parsing raw_score at offset {parser.offset}: {e}")
        result['raw_score'] = {
    ...
    logger.info(f"Parsing normalized_score at offset {parser.offset}")
    try:
        if version >= 2:
        ...
        logger.error(f"Error parsing normalized_score at offset {parser.offset}: {e}")
        result['normalized_score'] = {k: 0.0 for k in result['raw_score'].keys()}

    parser.current_field = "total_score"
    logger.info(f"Parsing total_score at offset {parser.offset}")
    try:
        result['total_score'] = parser.read_f64()
    except Exception as e:
        logger.error(f"Error parsing total_score at offset {parser.offset}: {e}")
...
def bincode_stake(parser):
    ...
    try:
        ...
        logger.error(f"Error parsing stake.active at offset {parser.offset}: {e}")
        active = 0
    parser.current_field = "activating"
    try:
        activating = parser.read_u64()
    except Exception as e:
        logger.error(f"Error parsing stake.activating at offset {parser.offset}: {e}")
        activating = 0
    parser.current_field = "deactivating"
    try:
        deactivating = parser.read_u64()
    except Exception as e:
        logger.error(f"Error parsing stake.deactivating at offset {parser.offset}: {e}")
        deactivating = 0
    parser.current_field = None
    return {
        'active': active,
...
def bincode_score(parser, version, is_pool):
    ...
    for field in ['skip_rate', 'prior_skip_rate', 'subsequent_skip_rate', 'cu', 'latency', 'llv', 'cv']:
        ...
        try:
            ...
            logger.error(f"Error parsing score.{field} at offset {parser.offset}: {e}")
            score[field] = 0.0
    parser.current_field = "vote_inclusion"
    try:
        score['vote_inclusion'] = parser.read_f64() if version >= 1 else 0.0
    except Exception as e:
        logger.error(f"Error parsing score.vote_inclusion at offset {parser.offset}: {e}")
    ...
    try:
        ...
        logger.error(f"Error parsing score.apy at offset {parser.offset}: {e}")
        score['apy'] = 0.0
...
def bincode_noneligibility_reason(parser):
    ...
    try:
        ...
        logger.error(f"Error parsing noneligibility_index at offset {parser.offset}: {e}")
        return None
    if index == 0:
        ...
        try:
            ...
            logger.error(f"Error parsing blacklist_reason at offset {parser.offset}: {e}")
            return None
    elif index == 1:
        ...
        try:
            ...
            logger.error(f"Error parsing recent_epochs_leader at offset {parser.offset}: {e}")
            return None
    elif index == 3:
        ...
        try:
            ...
            logger.error(f"Error parsing recent_epochs_credits at offset {parser.offset}: {e}")
            return None
    elif index == 4:
        ...
        try:
            ...
            logger.error(f"Error parsing recent_epochs_delinquency at offset {parser.offset}: {e}")
            return None
    elif index == 5:
        ...
        try:
            ...
            logger.error(f"Error parsing commission at offset {parser.offset}: {e}")
            return None
    elif index == 7:
        ...
        try:
            ...
            logger.error(f"Error parsing recent_epochs_apy at offset {parser.offset}: {e}")
            return None
    elif index == 8:
        ...
        logger.error(f"Invalid non eligibility reason: {index} at offset {parser.offset}")
        return None
...
def insert_validator_data(vote_account_pubkey, validator_data, epoch, in_pool):
    if validator_data is None or validator_data['details'] is None:
        logger.warning(f"Skipping insertion for validator {vote_account_pubkey} due to incomplete data")
        return
    logger.info(f"Attempting to insert validator {vote_account_pubkey}, in_pool={in_pool}")
    ...
    try:
        ...
        logger.info(f"Successfully inserted validator {vote_account_pubkey}, in_pool={in_pool}")
        conn.commit()
    except Exception as e:
        logger.error(f"Error inserting data for validator {vote_account_pubkey}: {e}")
