...
DEBUG = False  # Set to False to disable debug printing
...
def save_chart_html(fig, chart_title, output_filename):
    """
    Saves a Plotly figure as a complete HTML page with a favicon
    and the Fueled by Trillium logo below the chart.
    """
        ...
        if epoch_aggregate_data is not None:
            if epoch > 0 and epoch_aggregate_data['total_active_stake'] is not None:  # was missing active stake prior to epoch 632 -- we have it now
                try:
                    ...
                    print(f"Error calculating avg_stake_per_leader_slot for epoch {epoch} in last ten epochs: {e}")
...
def generate_ten_epoch_validator_rewards(max_epoch, engine):
    ...
    for row in results:
        ...
        if DEBUG and record['identity_pubkey'] == '5pPRHniefFjkiaArbGX3Y8NUysJmQ9tMZg3FrFGwHzSm':
            print(f"Debug - Name: {record['name']}, "
                  f"Identity Pubkey: {record['identity_pubkey']}, "
                  f"Commission: {record['commission']}, "
                  f"Validator Inflation Reward: {record['validator_inflation_reward']}")
...
def calculate_stake_statistics(epoch, max_epoch, engine):
    try:
        ...
        try:
            ...
            filename = f'epoch{epoch}_stake_distribution_charts.html'
            save_chart_html(fig, "Solana Stake Distribution Charts", filename)

            if epoch == max_epoch:
                filename = 'stake_distribution_charts.html'
                save_chart_html(fig, "Solana Stake Distribution Charts", filename)
...
def calculate_stake_statistics_metro(epoch, max_epoch, engine):
    try:
        ...
        try:
            ...
            filename = f'epoch{epoch}_stake_distribution_charts_metro.html'
            save_chart_html(fig, "Solana Stake Distribution Charts (Metro)", filename)

            if epoch == max_epoch:
                filename = "stake_distribution_charts_metro.html"
                save_chart_html(fig, "Solana Stake Distribution Charts (Metro)", filename)
...
def plot_latency_and_consensus_charts(start_epoch, end_epoch, max_epoch, engine=None):
    ...
    # jrh -- 2025 we only use xshin for average_cv 
    query = """
    SELECT 
        vs.epoch, 
        vs.version, 
