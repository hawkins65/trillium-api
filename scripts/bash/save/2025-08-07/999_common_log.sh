...
# Configuration
# Ensure LOG_BASE_DIR is always set and exported
if [[ -z "$LOG_BASE_DIR" ]]; then
    export LOG_BASE_DIR="$HOME/log"
else
    export LOG_BASE_DIR
fi
# Ensure LOG_LEVEL is always set and exported
if [[ -z "$LOG_LEVEL" ]]; then
...
get_log_priority() {
    local level="$1"
    # Debug: Check what's being passed as level
    if [[ "$level" == *"/"* ]] || [[ -z "$level" ]]; then
        echo "WARNING: Invalid log level passed: '$level'" >&2
        echo "1"  # Return default priority
        return
    fi
