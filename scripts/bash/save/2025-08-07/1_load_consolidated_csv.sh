...
if echo $epoch_number | python3 "$TRILLIUM_SCRIPTS_PYTHON/91_load_consolidated_csv.py"; then
    ...
    bash "$DISCORD_NOTIFY_SCRIPT" error "$script_name" "Consolidated CSV loading" "echo $epoch_number | python3 $TRILLIUM_SCRIPTS_PYTHON/91_load_consolidated_csv.py" "$exit_code" "$epoch_number"
