...
SQL_FILES=(
    ...
    "vote_latency_by_asn_by_city.sql"
    "vote_latency_by_city.sql"
...
psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -f "$TRILLIUM_SCRIPTS_SQL/92_city-to-metro.sql"
...
for sql_file in "${SQL_FILES[@]}"; do
    ...
    psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -v epoch="$EPOCH" -f "$TRILLIUM_SCRIPTS_SQL/$sql_file" >> "$epoch_output_file"
    ...
    if [[ $? -eq 0 ]]; then
        ...
        bash $TRILLIUM_SCRIPTS_BASH/copy-pages-to-web.sh $epoch_output_file
        ...
        bash $TRILLIUM_SCRIPTS_BASH/copy-pages-to-web.sh $static_output_file
    else
