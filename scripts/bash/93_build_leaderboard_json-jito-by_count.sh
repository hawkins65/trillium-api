...
log "INFO" "🔍 Verifying HTML files were created"
if [ ! -f "$FILE_200" ] || [ ! -f "$FILE_350" ]; then
    ...
    echo "Expected files: $FILE_200 and $FILE_350"
...
log "INFO" "🌐 Copying $FILE_200 to web"
if bash copy-pages-to-web.sh "$FILE_200"; then
...
if cp "$FILE_200" "$FILE_200_DEFAULT"; then
...
if bash copy-pages-to-web.sh "$FILE_200_DEFAULT"; then
...
log "INFO" "🌐 Copying $FILE_350 to web"
if bash copy-pages-to-web.sh "$FILE_350"; then
...
if cp "$FILE_350" "$FILE_350_DEFAULT"; then
...
if bash copy-pages-to-web.sh "$FILE_350_DEFAULT"; then
